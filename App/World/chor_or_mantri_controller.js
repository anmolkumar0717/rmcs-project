...
export default class chor_or_mantri_Controller {
  constructor() {
    
    this.app = new App();
    this.scene = this.app.scene;
    this.physics = this.app.world.physics;
    this.character = this.app.world.chor_or_mantri.instance;
    this.avatar2 = this.app.world.chor_or_mantri.avatar;
    ...
    playerinfo.subscribe((state)=>{
      ...
      death.subscribe((state) => {
        this.hasDied = state.death; // Update death state
      });

  })

    this.autoMovementSpeed = 0.02; 
    this.autoDirection = new THREE.Vector3(-1, 0, -0.4); 
  ...
  loop() {
    ...
    if (this.character.position.x >= -6.4 && this.currentAnimationIndex === 0) {
      ...
      if (this.character.position.x === -6.4092024665903855) {
      ...
      if (this.movement1 === -32.000000000000014) {
